...
function RestaurantImageCard({
  ...
  const orderText = `Order #${image.display_order ?? index + 1}`;
  ...
  const handlePreview = () => {
    if (imageUrl) {
      onOpenPreview(
        imageUrl,
        `Restaurant Photo ${image.display_order ?? index + 1}`,
        orderText,
        uploadDate,
      );
    }
    ...
    >
      ...
      <CardContent className="p-3.5 flex items-center justify-between text-xs">
        ...
        <Button
          ...
          className="size-6 text-slate-400 hover:text-slate-900"
        >
...
export function RestaurantImagesTab({ images = [] }: RestaurantImagesTabProps) {
  ...
  return (
    <div className="space-y-6" data-testid="restaurant-images-tab">
      {/* Header */}
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2">
        <div>
          <div className="flex items-center gap-2.5">
            <h2 className="text-lg font-bold text-slate-900">Restaurant Gallery & Banners</h2>
            <Badge variant="secondary" className="font-mono text-xs">
              {images.length} Photos
            </Badge>
          </div>
          <p className="text-xs text-slate-500 mt-0.5">
            Storefront photos, ambiance images, and promotional display assets
          </p>
        </div>
      </div>
      ...
      {images.length === 0 ? (
        <Card className="border-slate-200/80 shadow-xs bg-white">
          <CardContent className="py-14 text-center">
            ...
            <p className="text-sm font-bold text-slate-800">No gallery photos uploaded</p>
            <p className="text-xs text-slate-500 mt-1 max-w-sm mx-auto">
              The restaurant has not uploaded any storefront or menu images yet.
            </p>
          </CardContent>
        </Card>
      ) : (
